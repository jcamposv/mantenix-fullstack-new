...
export function EditProductionLineForm({
  productionLine,
  sites,
}: EditProductionLineFormProps) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)

  const form = useForm<UpdateProductionLineFormData>({
    resolver: zodResolver(updateProductionLineSchema),
    defaultValues: {
      ...
      taktTime: productionLine.taktTime || undefined,
      unitPrice: productionLine.unitPrice || undefined,
    },
  })

  const onSubmit = async (data: UpdateProductionLineFormData) => {
    setIsSubmitting(true)

    try {
      const response = await fetch(`/api/production-lines/${productionLine.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Error al actualizar línea de producción')
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : 'Error al actualizar línea de producción'
      )
    } finally {

...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters = approvalRuleFiltersAPISchema.parse(rawFilters)

    const result = await ApprovalRuleService.getList(
      session,
      {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const approvalRule = await ApprovalRuleService.create(
    ...
    return NextResponse.json(
      {
        approvalRule,
        message: 'Regla de aprobación creada exitosamente'
      },
      { status: 201 }
    )
  } catch (error) {
